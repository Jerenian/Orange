    ...
    <div onClick={() => handleClick()} className={modal.visible ? classes.wrapper : classes.unwrapper}>
      <div onClick={(e) => childClick(e)} className={classes.container}>
        <div className={classes.inputContainer}>
            <input
            value={dataEdit.name}
            onChange={(e) => setData({...dataEdit, name: e.target.value})}
            placeholder='Название *' 
            type="text" />
            <input
            value={dataEdit.price}
            onChange={(e) => setData({...dataEdit, price: e.target.value})}
            placeholder='Цена *' 
            type="number" />
            <input
            value={dataEdit.description}
            onChange={(e) => setData({...dataEdit, description: e.target.value})}
            ...
            <button onClick={() => uploadClick()} className={classes.upload}> 
                ...
                onChange={(e) => setData({...dataEdit, file: e.target.files})}
                type="file"
                 />
            ...
            <div>{dataEdit.file === modal.img ? <p style={{background: "red"}} >Изображение не заменено</p> : <p style={{background: "green"}}>Вы изменили изображение</p>}</div>
        </div>
        <div className={classes.saveContainer}>

            <div className={classes.popular}>
                {dataEdit.isPopular ? (
                  <div className={classes.isPopular}>
                    Данный товар добавлен в категорию популярных 
                    <button onClick={() => 
                    setData({...dataEdit, isPopular: false})}
                    className={classes.PopularBtn}>
                      Убрать
                    </button>
                  </div>
                ) : (
                  <div className={classes.NoPopular}>
                    Данный товар не добавлен в категорию популярных 
                    <button onClick={() =>
                      setData({...dataEdit, isPopular: true})} 
                      className={classes.PopularBtn}>
                        Сделать
                      </button>
                  </div>
            ...
            <button disabled={data ? true : false} onClick={() => saveClick()} className={data ? classes.saved : classes.save}>
                {isLoading ? (
                  <span>загрузка</span>
                ) : data ? (
                <div>Сохранено!</div>
