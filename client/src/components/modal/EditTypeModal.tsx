...
const EditTypeModal = () => {
    ...
    const [data, setData] = useState<ICreateType>({name: "", file: undefined})
    const edit = useChangeTypeMutation()
    console.log(data)
    useEffect(()=> {
        setData({...data, name: modal.name, file: modal.img})
    },[modal])
    ...
    const saveClick = async () => {
        const dataType = await edit(data)
    ...
    <div onClick={() => handleClick()} className={modal.visible ? classes.wrapper : classes.unwrapper}>
      <div onClick={(e) => childClick(e)} className={classes.container}>
        ...
        <div className={classes.saveContainer}>
            <button  disabled = {modal.name === data.name && modal.img === data.file} onClick={() => saveClick()} className={classes.save}>
                Сохранить
            </button>
        </div>
      </div>
