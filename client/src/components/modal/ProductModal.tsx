...
const ProductModal = () => {
    ...
    const handleClick = () => {
        dispatch(changeProductModal(null))
    }
    ...
    const saveClick = async () => {
      console.log(dataProduct.palette)
      dataProduct.typeId = id
        if(!dataProduct.file || !dataProduct.name || !dataProduct.price) {
      ...
      <div onClick={(e) => childClick(e)} className={classes.container}>
        <div className={classes.inputContainer}>
            ...
            <button onClick={() => uploadClick()} className={classes.upload}> 
                ...
                onChange={(e) => setDataProduct({...dataProduct, file: e.target.files})}
                type="file"
                 />
            ...
            <div>{error.messaage ?  <p>{error.messaage}</p>: null }</div>
            <div>{dataProduct.file ? <p>Изображение загружено</p> : null}</div>
        </div>
