  ...
  return (
    <div onClick={() => handleClick()} className={modal ? classes.wrapper : classes.unwrapper}>
      <div onClick={(e) => childClick(e)} className={classes.container}>
        <h2 className={classes.title}>Создание новой категории товаров</h2>
        <div className={classes.inputContainer}>
            <input
            onChange={(e) => setData({...dataType, name: e.target.value})}
            placeholder='Введите название категории'
            type="text" />
            <button onClick={() => uploadClick()} className={classes.upload}> 
                <input 
                ref={inputRef}
                onChange={(e) => setData({...dataType, file: e.target.files})}
            ...
            <div>{dataType.file ? <p>Изображение загружено</p> : null}</div>
        </div>
